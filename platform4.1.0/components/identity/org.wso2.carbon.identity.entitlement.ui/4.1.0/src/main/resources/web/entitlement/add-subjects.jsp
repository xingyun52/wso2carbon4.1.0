...
<%
    ...
    if(matchId != null && !matchId.trim().equals("") && attributeValue != null &&
        ...
        matchElementDTO.setMatchElementName(EntitlementPolicyConstants.SUBJECT_ELEMENT);
        ...
        if(attributeId != null && !attributeId.trim().equals("") &&
           ...
           !attributeDesignatorDataType.trim().
            ...
            designatorDTO.setElementName(EntitlementPolicyConstants.SUBJECT_ELEMENT);
            ...
            designatorDTO.setSubjectCategory(subjectCategory);
...
<script type="text/javascript">
    ...
    function deleteMatchElement(matchElementId) {
        location.href = 'delete-match.jsp?subElementName=Subject&matchElementId=' + matchElementId +
                        '&editSubElement=' + <%=editSubElement%> ;
    }

    function doCancel() {
        location.href = 'cancel-match.jsp?subElementName=Subject';
    }

    function doCancelMatch() {
        location.href = 'add-subjects.jsp?editSubElement=' + <%=editSubElement%>;
    }

    function doAdd () {
        var subElementId = "<%=entitlementPolicyBean.getCurrentSubElementNumber()%>";
        location.href = 'add-target.jsp?subElementName=Subject&subElementId=' + subElementId +
                        '&editSubElement=' + <%=editSubElement%>;
    }
    ...
    function doAddMatch () {
        ...
        if(doValidation()){
            document.matchElementForm.action = "add-subjects.jsp?matchElementId=" + '<%=matchElementId%>'
...
<div id="middle">
    ...
    %>
        <h3 class="page-subtitle"><fmt:message key='edit.subject.element'/></h3>    
    <%
        } else {
    %>
        <h3 class="page-subtitle"><fmt:message key='add.subject.element'/></h3>
    <%
	    }
    %>
            ...
            <div id="attributeDesignatorLinkRow" style="margin-left:2px; <%if(attributeDesignatorDTO == null) {%>display:none<% } %>">
                <table class="noBorders" cellspacing="0" style="width:100%;padding-top:5px;">
                    ...
                    <tr>
                        ...
                        %>
                        <td><input id="subjectCategory" name="subjectCategory" class="text-box-big" type="text"
                                   value="<%=attributeDesignatorDTO.getSubjectCategory()%>"></td>
                        <%
                            } else {
                        %>
    ...
    <div class="tabTen">
        ...
        <tr>
            <td><fmt:message key="subject.match"/>  <%=existingMatchElement.getElementId()%></td>
            <td><%=existingMatchElement.getMatchId()%></td>
            <td>
                <a href="add-subjects.jsp?subElementName=Subject&matchElementId=<%=existingMatchElement.getElementId()%>&editMatchElement=true&editSubElement=<%=editSubElement%>" class="icon-link" style="background-image:url(images/edit.gif);"><fmt:message key="edit"/></a>
        ...
        %>
        <tr class="noDataBox">
            <td colspan="3"><fmt:message key="no.subject.match.define"/><br/></td>
        </tr>
        <%
            }
        %>
