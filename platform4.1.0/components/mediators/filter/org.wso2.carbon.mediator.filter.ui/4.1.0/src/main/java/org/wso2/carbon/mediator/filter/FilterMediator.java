...
public class FilterMediator extends AbstractListMediator {
    ...
    public OMElement serialize(OMElement parent) {
        ...
        if (getSource() != null && getRegex() != null) {
            SynapseXPathSerializer.serializeXPath(getSource(), filter, "source");
            filter.addAttribute(fac.createOMAttribute(
                "regex", nullNS, getRegex().pattern()));
        } else if (getXpath() != null) {
            SynapseXPathSerializer.serializeXPath(getXpath(), filter, "xpath");
        } else {
            // TODO exception
        }
    ...
    public void build(OMElement elem) {
        ...
        if (attXpath != null) {
            if (attXpath.getAttributeValue() != null &&
                ...
                try {
                    setXpath(SynapseXPathFactory.getSynapseXPath(elem, ATT_XPATH));
