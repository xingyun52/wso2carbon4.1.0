...
public class APIStoreHostObject extends ScriptableObject {
    ...
    public static String jsFunction_getKey(Context cx, Scriptable thisObj,
        ...
        if (!(args[7] instanceof String)) {
            HostObjectUtil.invalidArgsError(hostObjectName, methodName, "8", "string", args[7], false);
        }
    ...
    public static NativeObject jsFunction_getApplicationKey(Context cx, Scriptable thisObj,
        ...
        try {
            ...
            boolean first = true;
            for (int i = 0; i < accessAllowDomainsArray.length; i++) {
                if (first) {
                    authorizedDomains = accessAllowDomainsArray[i];
                    first = false;
                } else {
                    authorizedDomains = authorizedDomains + ", " + accessAllowDomainsArray[i];
                }
            }
