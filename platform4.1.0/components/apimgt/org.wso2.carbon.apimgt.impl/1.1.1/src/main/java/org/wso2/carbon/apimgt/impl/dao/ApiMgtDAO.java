...
public class ApiMgtDAO {
    ...
    public String refreshAccessToken(String keyType, String oldAccessToken,
        ...
        String consumerKey = findConsumerKeyFromAccessToken(oldAccessToken);
    ...
    public static String getAuthorizedDomains(String accessToken) throws APIManagementException {
        ...
        String accessAllowDomainsSql = "SELECT a.AUTHZ_DOMAIN " +
                ...
                " INNER JOIN IDN_OAUTH2_ACCESS_TOKEN b " +
	...
	private String getUserfromEmail(String email) {
		...
		String username = null;
		  try {
				...
				log.error("Error while retriivng the username using the email : "+email, e);
			}
		return username;
	}
	
	/**
	 * identify the loggedin user
	 * @param userID
	 * @return
	 */
	private String getLoginUserName(String userID){
		...
		if (isUserLoggedInEmail(userID)) {
			loginUserName = getUserfromEmail(userID);
		}
    ...
    private long getApplicationAccessTokenValidityPeriod() {
        ...
        try {
            ...
            String expireTimeString = config.getFirstProperty(APIConstants.API_KEY_MANAGER_APPLICATION_ACCESS_TOKEN_VALIDATION_PERIOD);
            if (expireTimeString != null) {
                expireTime = Long.parseLong(expireTimeString);
            }else{
                expireTime=OAuthServerConfiguration.getInstance().getDefaultAccessTokenValidityPeriodInSeconds();
            }
        } catch (Exception e) {
