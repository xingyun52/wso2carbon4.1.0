    ...
    jagg.post("/site/blocks/stats/ajax/stats.jag", { action:"getAPIUsageByResourcePath", currentLocation:currentLocation},
              function (json) {
                  if (!json.error) {
                      ...
                      for (var i = 0; i < json.usage.length; i++) {
                          $('#resourcePathUsageTable').append($('<tr><td>' + json.usage[i].apiName + '</td><td>' + json.usage[i].version + '</td><td>' + json.usage[i].context + '</td><td>' + json.usage[i].method + '</td><td>' + json.usage[i].count + '</td></tr>'));
                      }
                      if (length == 0) {
