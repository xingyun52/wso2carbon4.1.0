...
//                persistParameter(serviceGroupId, pathParam, serviceXPath);
            ...
            if (isHttpsTransportOnly(policy)) {
                ...
                try {
                    ...
                    serviceGroupFilePM.rollbackTransaction(serviceGroupId);
                    String msg = "Error while persisting https only policy info - " + service.getName();
                    log.error(msg);
                    throw new AxisFault(msg,e);
                }
            } else {
                // Must not enable all the transports just when adding a non-https policy -
                // Let the user manually add the needed transports.
