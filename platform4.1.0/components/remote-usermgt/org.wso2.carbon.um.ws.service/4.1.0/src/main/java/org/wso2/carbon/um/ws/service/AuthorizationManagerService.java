...
public class AuthorizationManagerService extends AbstractAdmin {
    ...
    public void authorizeRoleForResources(String roleName,PermissionDTO[] permissions) throws UserStoreException {
        List<PermissionDTO> authorizedResources=new ArrayList<PermissionDTO>();
        for(PermissionDTO permission:permissions){
            try{
            Util.checkAccess(permission.getResourceId());
            authorizedResources.add(permission);
            }catch (UserStoreException e){
                log.warn("Access denied to modify resources "+permission.getResourceId(),e);
            }
        }
        getAuthorizationManager().authorizeRole(roleName,authorizedResources.toArray(new Permission[authorizedResources.size()]));
    }
