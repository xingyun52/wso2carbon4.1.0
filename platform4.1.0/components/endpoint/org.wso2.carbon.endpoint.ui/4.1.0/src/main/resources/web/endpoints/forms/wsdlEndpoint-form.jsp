...
<script type="text/javascript">
    ...
    window.location.href = "wsdlEndpoint.jsp";
</script>
<%
                }
            }
    ...
    if (endpoint != null) {
        ...
        if (endpoint.getTimeoutAct() != null) {
            if (endpoint.getTimeoutAct().equals("discard")) {
                ...
                actionDuration = Integer.parseInt(endpoint.getTimeoutActionDuration());
...
<table class="normal-nopadding">
    ...
    <tr style="<%=!isAnonymous?"":"display:none"%>">
        <td width="250px"><fmt:message key="endpointName"/> <span
                class="required">*</span></td>
        <td><input id="endpointName" name="endpointName"
                   value="<%=name%>"
                <%=(!"".equals(name)) ? "disabled=\"disabled\"" : ""%>
    ...
    <tr>
        <td class="leftCol-small"><fmt:message key="service"/> <span
                class="required">*</span></td>
        <td><input id="wsdlendpointService" name="wsdlendpointService"
                   value="<%=service%>"
                <%=isFromTemplateEditor ? "" : "onkeypress=\"return validateText(event);\""%> />
        </td>
    </tr>
    <tr>
        <td><fmt:message key="port"/> <span
                class="required">*</span></td>
        <td><input id="wsdlendpointPort" name="wsdlendpointPort"
                   value="<%=port%>"
                <%=isFromTemplateEditor ? "" : "onkeypress=\"return validateText(event);\""%> />
        </td>
    </tr>
...
<tr>
    ...
    <td><input type="text" id="retryTimeOut" name="retryTimeOut"
               value="<%=retryDelayTimeOut==0?EndpointConfigurationHelper.getMappingFrom(templateMappings, TemplateParameterContainer.EndpointDefKey.retriesOnTimeoutBeforeSuspend):retryDelayTimeOut%>"/>
    </td>
</tr>
<tr>
...
<tr>
    ...
    <td><input type="checkbox"
               onchange="showHideOnSelect('wsAddressing','tr_separate_listener')"
...
<tr>
    ...
    <td><input type="checkbox"
               onclick="showHideOnSelect('wsSecurity','tr_ws_sec_policy_key')"
...
<tr>
    ...
    <td><input onclick="showHideOnSelect('wsRM','tr_ws_rm_policy_key')" type="checkbox"
               id="wsRM" name="wsRM" value="wsRM"
...
<tr>
    ...
    <td>
        <textarea name="endpointDescription" id="endpointDescription"
