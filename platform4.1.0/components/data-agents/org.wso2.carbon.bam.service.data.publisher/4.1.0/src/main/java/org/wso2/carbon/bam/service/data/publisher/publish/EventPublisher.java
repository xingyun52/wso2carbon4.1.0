...
public class EventPublisher {
    ...
    public void publish(Event event, EventConfigNStreamDef configData) {
        ...
        if (event.getStatisticsType().equals(StatisticsType.ACTIVITY_STATS)) {
            key = configData.getUrl() + "_" + configData.getUserName() + "_" +
                  configData.getPassword() + "_" + StatisticsType.ACTIVITY_STATS.name();
            eventPublisherConfig = ServiceAgentUtil.getEventPublisherConfig(key);
            streamDef = configData.getActivityStreamDefinition();
        } else if (event.getStatisticsType().equals(StatisticsType.SERVICE_STATS)) {
            key = configData.getUrl() + "_" + configData.getUserName() + "_" +
