...
public class CGAgentServiceComponent {
    ...
    protected void activate(ComponentContext context) {
        ...
        try {
            ...
            authorizationManager.clearRoleActionOnAllResources(CGConstant.CG_PUBLISH_ROLE_NAME,
                    UserMgtConstants.EXECUTE_ACTION);
            authorizationManager.clearRoleActionOnAllResources(CGConstant.CG_UNPUBLISH_ROLE_NAME,
                    UserMgtConstants.EXECUTE_ACTION);
            for (String permission : publishOptimizedList) {
                authorizationManager.authorizeRole(CGConstant.CG_PUBLISH_ROLE_NAME, permission,
                        UserMgtConstants.EXECUTE_ACTION);
            }

            for (String permission : unPublishOptimizedList) {
                authorizationManager.authorizeRole(CGConstant.CG_UNPUBLISH_ROLE_NAME, permission,
                        UserMgtConstants.EXECUTE_ACTION);
            }
            ...
            if (!manager.isExistingRole(CGConstant.CG_PUBLISH_ROLE_NAME)) {
                manager.addRole(CGConstant.CG_PUBLISH_ROLE_NAME, new String[]{realm.getRealmConfiguration().getAdminUserName()}, null);
            }

            if (!manager.isExistingRole(CGConstant.CG_UNPUBLISH_ROLE_NAME)) {
                manager.addRole(CGConstant.CG_UNPUBLISH_ROLE_NAME, new String[]{realm.getRealmConfiguration().getAdminUserName()}, null);
            }
