...
public class ServerUtils {
    ...
    public synchronized void startServerUsingCarbonHome(String carbonHome, String carbonFolder,
        ...
        Process tempProcess;
        isCoverageEnable = new EnvironmentBuilder().getFrameworkSettings().getCoverageSettings().getCoverageEnable();
        try {
            if (isCoverageEnable) {
...
//            FrameworkSettings.init();
    ...
    public synchronized void shutdown(int portOffset, FrameworkProperties properties)
            throws Exception {
        if (process != null) {
            if (ClientConnectionUtil.isPortOpen(defaultHttpsPort,
...
//                serviceBaseURL = "https://localhost:" + httpsPort + "/services/";
                sessionCookie = new LoginLogoutUtil(httpsPort, properties.getProductVariables().
                        getHostName()).login(userInfo.getUserName(), userInfo.getPassword(),
                                             properties.getProductVariables().getBackendUrl());
            } else {
//                serviceBaseURL = "https://localhost:" + httpsPort + "/" + carbonManagementContext + "/services/";
            ...
            ServerAdminClient serverAdminClient =
                    new ServerAdminClient(properties.getProductVariables().getBackendUrl(),
    ...
    public synchronized void restartGracefully(ServerAdminClient serverAdminClient,
        ...
        /*URL backendURL = new URL("https://localhost:9443/test");
        backendURL.getHost();
        backendURL.getPort();*/
        if (isCoverageEnable) {
