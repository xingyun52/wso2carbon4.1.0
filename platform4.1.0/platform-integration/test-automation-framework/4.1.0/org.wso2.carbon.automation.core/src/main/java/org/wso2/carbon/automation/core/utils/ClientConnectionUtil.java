...
public final class ClientConnectionUtil {
     ...
     */
    public static void waitForLogin(int port, String hostName, String backendURL) {
        ...
        while (loginFailed && (System.currentTimeMillis() - startTime) < TIMEOUT) {
            ...
            try {
                ...
                new LoginLogoutUtil(port, hostName).login(userInfo.getUserName(), userInfo.getPassword(), backendURL);
                loginFailed = false;
                return;
            } catch (Exception e) {
