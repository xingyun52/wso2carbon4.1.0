...
/*      String[] command = cmd.split(Utils.TENANT_ID_SEPARATOR_CHAR_SEQ);
    ...
    public void clean() {
      ...
      boolean retainHistoryLogs = session.getConf().
              getBoolean(HiveConf.ConfVars.HIVERETAINHISTORYLOGS.varname, false);

      if (!retainHistoryLogs) {
        File historyFile = new File(session.getHiveHistory().getHistFileName());
        historyFile.delete();
      }
