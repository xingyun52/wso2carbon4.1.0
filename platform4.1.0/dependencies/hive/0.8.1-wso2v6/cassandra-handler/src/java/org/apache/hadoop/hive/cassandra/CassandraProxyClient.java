...
public class CassandraProxyClient implements java.lang.reflect.InvocationHandler {
  ...
  private void initializeConnection(String keyspace, Map<String, String> credentials) throws CassandraException {
    ...
    for (String cassandraHost : hosts) {
        try {
            clientHolder = createConnection(cassandraHost, keyspace, credentials);

            if (logger.isDebugEnabled()) {
                logger.debug("Connected to cassandra at " + cassandraHost + ":" + port);
            }

            break;
        } catch (CassandraException e) {
            logger.error("Error while trying to connect to cassandra host:"+cassandraHost, e);
        }
    }
