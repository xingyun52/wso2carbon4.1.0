...
import org.apache.hadoop.hive.serde2.lazy.objectinspector.LazyCassandraMapObjectInspector;
    ...
    if (getFields() == null) {
      ...
      for (int i = 0; i < getFields().length; i++) {
        ...
        if (cassandraColumn.endsWith(":")) {
          // want rest of the columns as a map
  ...
  private Object uncheckedGetField(int fieldID) {
    if (!getFieldInited()[fieldID]) {
      ...
      if (columnName.endsWith(":")) {
        ((LazyCassandraCellMap)obj).init(columnMap, cassandraColumnsBB);
