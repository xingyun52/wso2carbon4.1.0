...
public class DefineTableTestCase {
    ...
    public void testCreatingSQLTableDefinition() {
        TableDefinition tableDefinition = QueryFactory.createTableDefinition();
        tableDefinition.name("cseEventStream").attribute("symbol", Attribute.Type.STRING).attribute("price", Attribute.Type.INT).attribute("volume", Attribute.Type.FLOAT).from("SQL","myDB","fooTable");
