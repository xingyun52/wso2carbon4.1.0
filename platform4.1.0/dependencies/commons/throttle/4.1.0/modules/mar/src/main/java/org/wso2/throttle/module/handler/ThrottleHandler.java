...
import org.wso2.throttle.ReverseDNSUtil;
        ...
        if (canAccess) {
            ...
            if (messageContext.getFLOW() == MessageContext.IN_FLOW) {
                ...
                if (request != null) {
                	...
                	domain = (String) messageContext.getProperty(MessageContext.REMOTE_ADDR);
				...
				// domain name or not
				/*
				 * Note : It's very unlikely to receive domain name from domain
				 * = 'request.getRemoteHost()' call. Hence need to perform
				 * reverse DNS look-up to find domain name but it's very
				 * expensive
				 */
				String callerId = null;
				if (domain != null) {
					// loads the ThrottleContext
					ThrottleContext context = throttle
							.getThrottleContext(ThrottleConstants.DOMAIN_BASED_THROTTLE_KEY);
					if (context != null) {
						// Loads the ThrottleConfiguration
						ThrottleConfiguration config = context
								.getThrottleConfiguration();
						// We should find DNS name only if user have specify it.
						boolean needToProcess = false;
			if (config != null) {
			   if (config instanceof DomainBaseThrottleConfiguration) {
				needToProcess = ((DomainBaseThrottleConfiguration) config)
						.getConfigurationsMapSize() > 0 ? true
										: false;
							}
						}
                        if (needToProcess) {
                            //check for configuration for this caller
                            // Find DNS name before call this method
                            domain = convertToDNSName(domain, messageContext);
                            //check for configuration for this caller
...
//                roleID = Utils.extractCustomerKeyFromAuthHeader(oAuthHeader);
    ...
    private String convertToDNSName(String domain, MessageContext messageContext) {
		
		//1. First check weather DNS name is already found and available on MessageContext.
		Object domainNameProperty =  messageContext.getProperty(ThrottleConstants.DOMAIN_NAME_PROPERTY);
		if(domainNameProperty != null ){
			return (String) domainNameProperty;
		}
		
		//2. Name noy found, perform reverse DNS look up.		
		if (domain != null) {
			if (debugOn) {
				log.debug(" invorked reverse DNS look-up for  " + domain);
			}
			try {
				String name = ReverseDNSUtil.ReverseDNS(domain);	
				//Removing trailing dot if exists. 
				if( name != null && name.endsWith(".")){
					name = name.substring(0, name.length()-1);
				}
				messageContext.setProperty(ThrottleConstants.DOMAIN_NAME_PROPERTY, name);
				if (debugOn) {
					log.debug("reverse DNS look-up : " + domain + " = " + name);
				}
				return name;
			} catch (IOException e) {
				// ignore
				if (debugOn) {
					log.error("Error during convertToDNSName ", e);
				}
			}
		}
		return domain;
