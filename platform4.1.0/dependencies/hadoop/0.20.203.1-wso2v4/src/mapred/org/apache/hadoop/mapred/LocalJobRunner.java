...
class LocalJobRunner implements JobSubmissionProtocol {
  ...
  private class Job extends Thread
    ...
    public void run() {
      ...
      try {
        ...
        JobEndNotifier.localRunnerNotification(job, status);
          ...
          try {
          Path stagingDirectory = systemJobFile.getParent().getParent();  // sytemJobFile = [$CARBON_HOME/tmp]/hadoop/local/staging/{user}{jobid}/.staging/{job.attempt}/job.xml
    ...
    public void cleanUpForceFully() {
        try {
          ...
          // Ignore since this is most probably caused since the file has been already cleaned up by job thread
        }
  ...
  public void killJob(JobID id) {
    ...
    if (!jobs.get(id).isCleanedUp()) {
        try {
            jobs.get(id).join(30000); // Wait 30 seconds until the job thread cleans up its temporary files
        } catch (InterruptedException e) {
            jobs.get(id).cleanUpForceFully(); // try to cleanup temporary files forcefully
        }
