...
public class ListenerContext {
    ...
    public void build() throws AxisFault {
        ...
        List<Parameter> transportParams = transportIn.getParameters();
        Iterator<Parameter> itr = transportParams.iterator();
        while (itr.hasNext()) {
            Parameter p = itr.next();
            if ("port".equals(p.getName())) {
                p.setValue(Integer.toString(port));
                p.getParameterElement().setText(Integer.toString(port));
                break;
            }
        }
