...
public class ClientConnFactoryBuilder {
    ...
    public ClientConnFactoryBuilder parseSSL() throws AxisFault {
        ...
        final Parameter p = transportOut.getParameter("HostnameVerifier");
        final String s = p != null ? p.getValue().toString() : null;
        final X509HostnameVerifier hostnameVerifier;
        
        if ("Strict".equalsIgnoreCase(s)) {
            hostnameVerifier = ClientSSLSetupHandler.STRICT;
        } else if ("AllowAll".equalsIgnoreCase(s)) {
            hostnameVerifier = ClientSSLSetupHandler.ALLOW_ALL;
        } else if ("DefaultAndLocalhost".equalsIgnoreCase(s)) {
            hostnameVerifier = ClientSSLSetupHandler.DEFAULT_AND_LOCALHOST;
        } else {
            hostnameVerifier = ClientSSLSetupHandler.DEFAULT;
        }
        ssl = new SSLContextDetails(sslContext, new ClientSSLSetupHandler(hostnameVerifier));
