...
public class IterateWithRegistriesAsTargetEndpointsTestCase extends ESBIntegrationTest {
    ...
    public void testGovernanceEndPoint() throws Exception {
        ...
        URL url =
                getClass().getResource("/artifacts/ESB/mediatorconfig/iterate/iterateEndpoint.xml");
        endPointAdminClient.addDynamicEndPoint("gov:/myEndpoint/iterateEndpoint", new DataHandler(url));
    ...
    public void testConfigurationEndPoint() throws Exception {
        ...
        URL url =
                getClass().getResource("/artifacts/ESB/mediatorconfig/iterate/iterateEndpoint.xml");
        endPointAdminClient.addDynamicEndPoint("conf:/myEndpoint/iterateEndpoint", new DataHandler(url));
        String response = client.getMultipleResponse(getMainSequenceURL(), "WSO2", 2);
        Assert.assertNotNull(response);
        OMElement envelope = client.toOMElement(response);
        OMElement soapBody = envelope.getFirstElement();
