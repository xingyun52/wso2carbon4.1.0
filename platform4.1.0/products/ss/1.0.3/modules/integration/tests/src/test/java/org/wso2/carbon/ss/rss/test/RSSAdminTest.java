...
public class RSSAdminTest {
    ...
    public void testDbConnection() throws RSSAdminRSSManagerExceptionException, RemoteException {
        rssAdminStub.testConnection(rssTestHelper.getDataSource().get_dbDriverName(), rssTestHelper.getDataSource().getDbUrl(), rssTestHelper.getDataSource().getDbUser(), rssTestHelper.getDataSource().getRssDbPassword());
        assertTrue(true);
    ...
    private boolean isDatabasePrivilegeTemplateExists(String databasePrivilegeTemplateName)
        ...
        if(databasePrivilegeTemplates!=null)
        {
    ...
    @Test(dependsOnMethods = {"createDatabase","createDatabaseUser"},description = "Drop database user")
